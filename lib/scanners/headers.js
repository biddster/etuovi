    ...
    scan(host, config) {
        ...
        const promises = _.map(config.paths, (spec) => {
            ...
            return rp(options).then((response) => {
                l.verbose(response);
                const summary = [`${options.method} [${options.uri}] actual [${response.statusCode}] expected [${spec.expect}]`];
                const headers = _.assign({}, baseHeaders, _.mapKeys(spec.headers, (value, header) => header.toLowerCase()));
                _.forOwn(headers, (headerValueRegex, headerName) => {
                    const headerValue = response.headers[headerName];
                    if (!headerValue) {
                        summary.push(`${options.method} [${options.uri}] missing header [${headerName}]`);
                    } else if (!headerValue.match(new RegExp(headerValueRegex, 'i'))) {
                        summary.push(`${options.method} [${options.uri}] header [${headerName}] invalid value [${headerValue}]`);
                    } else {
                        summary.push(`${options.method} [${options.uri}] header [${headerName}] valid value [${headerValue}]`);
                    }
                });

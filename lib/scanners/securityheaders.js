...
module.exports = {
    scan(host, config) {
        ...
        const promises = _.map(paths, (path) => {
            return Promise.resolve(headers(new URI().scheme('https').host(host).path(path).toString(), true, true)).then(report => {
                l.verbose(report);
                const missingHeaders = _.keys(report.missingHeaders);
                const warnings = _.keys(report.warnings);
                return {
                    summary: [`[${path}] Score [${report.score}] found [${missingHeaders.length}] missing headers [${missingHeaders}] and [${warnings.length}] warnings [${warnings.join(',')}]`],
                    detail: report,
